...
Character Setup uses this active step order: Concept, Attributes, Skills, Edges, Hindrances, Powers, Gear, and Review.
...
Review summarizes available setup data, import warnings, and setup source audit results. It is the only setup step that exposes character save/finalization controls. The source audit distinguishes records explained by canonical setup source fields from records that still need a GM/table exception note or a more specific setup workflow. Eligible created pre-advance characters can mark unexplained records as GM/table exceptions from Review; this writes `creationSource: "setup-gm-exception"`, `source`, `sourceDetail`, and a `setupExceptions` metadata record.

Finish Setup & Start Playing is available from Review. It saves the current setup character, marks `creation.finalized` as `true`, snapshots the full creation baseline for eligible created pre-advance characters, and opens Combat. Finishing setup warns about incomplete core setup sections, including missing source-tracked starting Edge slots, but allows the player to start playing and return to Character Setup later. Invalid source-tracked starting Edges on editable pre-advance created characters are a blocking setup review issue, not an incomplete-section warning.
...
- Character Setup shell for created and imported characters, with Concept, Attributes, Skills, Edges, Hindrances, Powers, Gear, and Review steps. `setupStatus` decides whether setup review is needed.
...
- Basic setup exception marking from Review for eligible created pre-advance characters, with metadata persisted on the affected record and in `setupExceptions`.

### Partially implemented

- Setup source auditing is implemented for the current MVP shape. Review shows explained setup records and can mark unexplained records as GM/table exceptions. Richer exception notes and structured free/source-granted gear workflows are deferred.
...
- Show clear Review actions: save character, finish or confirm setup, and return to play when ready.
