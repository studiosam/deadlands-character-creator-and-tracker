...
Gear follows Powers and is the final setup step. It audits recorded money, weapons, armor, general gear, consumables, ammunition, vehicles, carried/on-body items, equipped/worn items, dropped items, stored/off-person items, container contents, current load, combat load, carrying capacity, and obvious missing or unknown item data. For characters created in this tool with no recorded Advances, it supports buying catalog weapons, ammunition, gear/armor, and vehicles from remaining setup funds in that order, while keeping current inventory visible beside the purchase controls. Ammo options for purchased weapons are prioritized and labeled; in the live Inventory tab, matching reserve ammo is bought from the weapon's own inventory card rather than a standalone ammo purchase form. Setup-created purchases reduce `moneyCents`, are source-tagged as `setup-starting-gear`, and preserve purchase metadata in `sourceDetail`. Source-tagged setup purchases can be sold back during setup, which refunds their recorded setup cost and removes the setup-created item when safe. Imported and advanced characters remain audit-only because their equipment may include post-creation purchases, loot, or table adjustments. Gear contains the setup finalization area and compact blocker list with jump actions. Free/granted and organization gear modeling remain later slices.
...
Canonical setup source fields are `creationSource`, `source`, and `sourceDetail`. Current setup source tokens include `setup-starting-hindrance`, `human-free-edge`, `hindrance-benefit`, `setup-starting-power`, `setup-arcane-background`, `setup-starting-gear`, and `setup-starting-funds`. Newly finalized created characters preserve those fields on the live records and in the creation baseline. Imported and advanced characters remain audit-only unless reliable setup-time data exists.
...
| Derived stats                                 | Keep on normal Character Sheet and move primarily to Tracker                           | Keep Pace, Parry, Toughness, Size, and Armor on the sheet as reference. Tracker remains the primary active-use location for Pace, Parry, Toughness, Wounds, Fatigue, and status.                                                     |
...
- Inventory owns gear, money, possessions, ammo, armor, vehicles, and storage.
  Live Inventory groups carried/on-body and backpack/container gear under Gear,
  shows matching firearm ammo on the weapon card, shows unmatched carried ammo
  as gear, and lists off-body home/cart/horse/camp items under Storage
  Locations.
