...
Finish Setup & Start Playing saves the current setup character, marks `creation.finalized` as `true`, snapshots the full creation baseline for eligible created pre-advance characters, and opens Combat. Finishing setup warns about incomplete core setup sections, including missing source-tracked starting Edge slots, but allows the player to start playing and return to Character Setup later. Invalid source-tracked starting Edges on editable pre-advance created characters are a blocking setup review issue, not an incomplete-section warning.
