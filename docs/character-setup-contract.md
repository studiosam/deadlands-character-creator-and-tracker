...
Character Setup uses this active step order: Concept, Attributes, Skills, Edges, Hindrances, Powers, and Gear. Gear is the final setup step.
...
Concept edits map to the active character's `name`, `gender`, `age`, `archetype`, `player`, `description`, and `background` fields and update through the normal setup persistence path as the user edits. Rank is not edited in Concept; it is shown as recorded or derived advancement context elsewhere. Attributes, Skills, Edges, Hindrances, Powers, and Gear now have functional setup controls for eligible characters. Each setup step before Gear ends with a Next button so the workflow is explicitly step-based. Save and finalization controls appear at Gear, where setup ends.

Race / Ancestry is not an active setup step in the current Deadlands-focused profile. It is read-only reference/audit data shown inside Concept. The built-in profile records Human as the supported race or ancestry and flags imported non-Human values for review instead of offering editing controls.

Hindrances supports selecting and removing Hindrances from the existing catalog, tracks starting Hindrance points, and spends counted benefit points on Attribute raises, extra starting Edge slots, Skill points, or extra starting money. Hindrances are optional. A character with no Hindrances may continue after explicitly choosing to continue without them; Gear finalization should not block merely because no Hindrance was selected. The Hindrances step follows the Edges step so players can draft the free Human Edge first, then use Hindrance benefits to cover extra starting choices. It presents selection first: Hindrance name, severity, and notes are entered together, selected Hindrances appear as cards, and benefit spending appears beneath those cards. Attribute raises, Skill points, and extra Edge slots bought or granted from Hindrances are assigned on the dedicated Attributes, Skills, and Edges steps; the source card that creates the bonus carries the shortcut button rather than embedding the full editors inside Hindrances. Minor is worth 1 point, Major is worth 2 points, and up to 4 points count for default starting benefits. Taking more than 4 points is allowed and remains complete; the UI shows an informational note that extra rewards require a table or GM exception. Benefit spending cannot exceed counted points, and spending reductions are blocked when the current starting build still depends on those points.

Attributes and Skills are editable for characters created in this tool that do not yet have recorded Advances. Attributes track the normal 5 Attribute points. Skills track 12 Skill points, Hindrance-granted extras, special Hindrance points such as Elderly's 5 Smarts-linked Skill points, baseline skills, and unskilled values at `d4-2`. New SWADE / Deadlands setup characters start with Athletics `d4`, Common Knowledge `d4`, Notice `d4`, Persuasion `d4`, Stealth `d4`, and Language `d8` for free. Those baseline skills do not consume setup Skill points. All other default setup skills start Unskilled unless purchased or granted, and Unskilled `d4-2` is not the same as trained `d4`. Edits update both the current character values and the stored creation baseline. Imported characters and advanced characters remain read-only audit views; recorded skills use the character's die values, and missing skills are shown as unskilled `d4-2`. Attribute cards and skill chips expose short usage notes through hover/focus help.

The current Deadlands-focused setup flow shows the default Weird West skill list and hides user-facing skills that are preserved only for data/import compatibility. `Psionics` is preserved in metadata but hidden from normal player-facing setup panels because it is not part of the default Weird West setup profile. Skills that are not in the current app catalog, such as `Electronics` or `Hacking`, are likewise not default Weird West setup options. If imported or custom data contains optional/non-default skills, that data should be preserved rather than silently discarded.

Edges audits recorded Edges against the current catalog and, for characters created in this tool with no recorded Advances, supports selecting the Human free Edge. Edge slots bought with Hindrance benefit points are also selected on the Edges step after the slot is purchased from Hindrances. Starting Edge choices are source-tagged as `human-free-edge` or `hindrance-benefit` so they remain distinct from later Advances. Imported and advanced characters remain audit-only. Arcane Background Edges are called out, and more than one Arcane Background Edge is flagged for review. Starting Edge selection, add-time validation, existing source-tracked setup Edge review, and setup confirmation use the same partial eligibility model: catalog match, legal Rank, recognized Trait die requirements, recognized prerequisite Edge names, no duplicate selected Edge, and available Human free or Hindrance benefit slot count. Invalid source-tracked setup Edges on editable pre-advance created characters mark Edges as Needs Review and block setup confirmation until removed or reselected. Missing starting Edge slots may remain incomplete under the current setup flow. Full complex Edge prerequisite enforcement, Advancement Edge prerequisite enforcement, and GM exception bookkeeping are deferred to later setup slices.

Powers follows Hindrances. It audits the recorded Arcane Background against `ARCANE_BACKGROUND_POWER_PROFILES`, including expected Arcane Skill and linked attribute, Power Points, expected starting power count, required starting powers, known power catalog matches, and obvious powers outside the matched Arcane Background power list. For characters created in this tool with no recorded Advances, it automatically grants the profile's starting Power Points and required starting Powers, then lets the player select any remaining legal starting Powers from the matched Arcane Background profile. Setup-created Power Points are source-tagged as `setup-arcane-background`, and setup-selected Powers are source-tagged as `setup-starting-power` so they remain distinct from later Advancement changes. Required starting powers are shown once as the selected required card, not duplicated as a separate checklist item. Non-arcane characters show this step as not applicable. Created pre-advance characters show missing Arcane Skill, Power Points, starting power count, or required starting powers as incomplete; imported and advanced characters remain audit-only and use Needs Review for the same gaps. Full Power effect automation and complex table-dependent Power legality remain deferred.

Gear follows Powers and is the final setup step. It audits recorded money, weapons, armor, general gear, consumables, ammunition, vehicles, carried/on-body items, equipped/worn items, dropped items, stored/off-person items, container contents, current load, combat load, carrying capacity, and obvious missing or unknown item data. For characters created in this tool with no recorded Advances, it supports buying catalog weapons, ammunition, gear/armor, and vehicles from remaining setup funds in that order, while keeping current inventory visible beside the purchase controls. Ammo options for purchased weapons are prioritized and labeled; ammo with no matching purchased weapon remains available but is clearly marked so the player knows it may be useless for the current loadout. Setup-created purchases reduce `moneyCents`, are source-tagged as `setup-starting-gear`, and preserve purchase metadata in `sourceDetail`. Source-tagged setup purchases can be sold back during setup, which refunds their recorded setup cost and removes the setup-created item when safe. Imported and advanced characters remain audit-only because their equipment may include post-creation purchases, loot, or table adjustments. Gear contains the setup finalization area, compact blocker list with jump actions, and the setup source audit/GM exception controls. Richer exception notes plus free/granted and organization gear modeling remain later slices.

Finish Setup & Start Playing is available from Gear. It saves the current setup character, marks `creation.finalized` as `true`, snapshots the full creation baseline for eligible created pre-advance characters, and opens the live tracker. Blocking setup issues disable finalization and appear near the final action with jump buttons to the relevant setup step. Warnings and optional notes do not block finalization. Invalid source-tracked starting Edges on editable pre-advance created characters are blocking setup issues, not optional review notes.

## Project Goal: Creation Baseline and Current Character

Characters created in this tool should track starting character creation choices separately from later character growth. The app snapshots a full `creationBaseline` for eligible finalized created characters, including starting Attributes, Skills, Hindrances, Edges, Powers, Power Points/resources, Gear, money, and setup exception placeholders. Later changes should be tracked through an advancement ledger. Current character values should be explainable from the creation baseline plus applied Advances and explicit table or GM exceptions.

Canonical setup source fields are `creationSource`, `source`, and `sourceDetail`. Current setup source tokens include `setup-starting-hindrance`, `human-free-edge`, `hindrance-benefit`, `setup-starting-power`, `setup-arcane-background`, `setup-starting-gear`, `setup-starting-funds`, and `setup-gm-exception`. Newly finalized created characters preserve those fields on the live records and in the creation baseline. Exception-marked records also create/update `setupExceptions` entries with the affected collection, record id, record type, display label, and timestamp. Imported and advanced characters remain audit-only unless reliable setup-time data exists.
...
- Character Setup shell for created and imported characters, with Concept, Attributes, Skills, Edges, Hindrances, Powers, and Gear steps. `setupStatus` decides whether setup review is needed.
...
- Created-character Hindrance selection, optional no-Hindrance continuation, Hindrance benefit spending, Attribute point spending, Skill point spending, starting Edge source tracking, and shared starting Edge save/confirm validation for eligible pre-advance characters.
- Starting Skill baseline model for eligible new SWADE / Deadlands characters: Athletics `d4`, Common Knowledge `d4`, Notice `d4`, Persuasion `d4`, Stealth `d4`, Language `d8`, and other default setup skills Unskilled until purchased or granted.
- Character Setup Powers selection for eligible pre-advance characters, including matched Arcane Background profile checks, required starting powers, legal starting power selection, and source-tracked starting Power Points.
- Character Setup Gear purchase tracking for eligible pre-advance characters, including catalog weapons, ammunition, gear, armor, vehicles, setup funds, setup purchase metadata, source tags, setup sell-back, useful-ammo prioritization, and compact current-inventory review.
- Full creation-baseline snapshots for finalized eligible created characters, including setup source fields and reload/export/import preservation.
- Basic setup exception marking from the Gear finalization audit for eligible created pre-advance characters, with metadata persisted on the affected record and in `setupExceptions`.

### Partially implemented

- Setup source auditing is implemented for the current MVP shape. Gear finalization shows explained setup records and can mark unexplained records as GM/table exceptions. Richer exception notes and structured free/source-granted gear workflows are deferred.
- Advancement has canonical ledger storage, adaptive forms, application helpers, validation, import history handling, safe undo checks where reliable, and focused browser coverage. The remaining product boundary question is whether the current Character-tab Advancement area should move behind a more deliberate Advancement workflow.
...
| Gear summary                                  | Move primarily to Inventory                                                            | Full gear, money, ammo, armor, weapons, vehicles, carried state, and add-item forms belong in Inventory. Character Setup Gear keeps source-tracked starting purchases for eligible created pre-advance characters.                   |
...
| Source badge and save/setup status indicators | Keep on normal Character Sheet and move detailed controls to Local Data or global menu | Keep a small source/setup-status indicator on the sheet. Detailed local save, import/export, and library management remain Local Data/global menu responsibilities.                                                                  |
...
- Show clear Gear finalization actions: save character, finish setup, and return to play when ready.
